...
function KanbanColumn({ 
  ...
  const columnStyles = isPersonal
    ? "bg-purple-50/50 dark:bg-purple-950/30 border-l-4 border-l-purple-500 border-t border-r border-b border-purple-200 dark:border-purple-800"
    : "bg-emerald-50/50 dark:bg-emerald-950/30 border-l-4 border-l-emerald-500 border-t border-r border-b border-emerald-200 dark:border-emerald-800";
  const headerStyles = isPersonal
    ? "border-purple-200 dark:border-purple-700"
    : "border-emerald-200 dark:border-emerald-700";
  const iconContainerStyles = isPersonal
  ...
  return (
    <div className={`flex-1 min-w-80 p-4 rounded-lg ${columnStyles}`}>
      <div className={`flex items-center gap-3 mb-4 pb-3 border-b ${headerStyles}`}>
        <div className={`p-2 rounded-lg ${iconContainerStyles}`}>
          <Icon className="w-4 h-4" />
        </div>
        <h4 className="font-semibold">{title}</h4>
        <span className={`ml-auto text-xs font-medium px-2.5 py-1 rounded-full ${badgeStyles}`}>{objectives.length}</span>
      </div>
